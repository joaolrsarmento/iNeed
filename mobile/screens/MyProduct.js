...
class MyProduct extends Component {
  ...
  componentDidMount(){
    ...
    let iOrThey = Number('canMailIt' in item);
    var result = mocks.ufs.find(obj => {
      return obj.uf == item.uf
    })
    console.log(result.uf)
    this.setState({
     title: item.title,
     description : item.description,
     valuePerDay : String(item.valuePerDay),
     city : item.city,
     uf : result.uf,
    iOrThey
    });

    if ('shouldMailIt' in item){
      this.setState({ mailIt: item.shouldMailIt});
    }else{
      this.setState({ mailIt: item.canMailIt, daysNeeded: Number(item.daysNeeded), from: item.startDate});
    }
  }
  ...
  handleDelete(){
    const item = this.props.navigation.state.params;
    if ('canMailIt' in item) api.delete(`giveitems/`+item.id);
    if ('shouldMailIt' in item) api.delete(`getitems/`+item.id);
    this.afterDelete();
  }
